	                    ...
	                    std::vector<sf::Sprite> const &t_wallSprites)
{
	...
	float newPosX = position.x + (std::cos(rotationRads) * m_speedInKm) * (t_dt / 1000);
	float newPosY = position.y + (std::sin(rotationRads) * m_speedInKm) * (t_dt / 1000);
	...
	return s_MAX_SPEED == m_speedInKm;
}
